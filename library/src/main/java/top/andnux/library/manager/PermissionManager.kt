...
object PermissionManager {
     ...
     * @param permission      权限
     */
    fun requestSingleEach(activity: FragmentActivity,
                          showAgainMsg: String,
                          notShowAgainMsg: String,
                          listener: OnPermissionListener?,
                          permission: String) {
        val rxPermissions = RxPermissions(activity)
        val disposable = rxPermissions.requestEach(permission)
     ...
     * @param args     权限组
     */
    fun requestMultipleEach(activity: FragmentActivity,
                            showMsg: String,
                            listener: OnPermissionListener?,
                            vararg args: String) {
        ...
        val rxPermissions = RxPermissions(activity)
        val disposable = rxPermissions.request(*args)
                .subscribe { aBoolean ->
                    if (aBoolean) {
                        //全部已经授权
                        listener?.onSucceed()
